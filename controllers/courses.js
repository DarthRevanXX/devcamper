...
exports.updateCourse = asyncHandler(async (req, res, next) => {
    ...
    if(course.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorResponse(`User ${req.user.id} is not authorized to update course ${course.id}`, 401))
    }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
    ...
    if(course.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(new ErrorResponse(`User ${req.user.id} is not authorized to delete course ${course.id}`, 401))
    }
